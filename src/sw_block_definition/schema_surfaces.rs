...
pub struct BuoyancySurfaces {
    #[serde(default)]
    pub surface: Vec<BuoyancySurface>,
}

#[derive(Serialize, Deserialize, Default, Debug)]
#[serde(default)]
pub struct BuoyancySurface {
    #[serde(rename = "@orientation")]
    pub orientation: Option<i32>,
    #[serde(rename = "@rotation")]
    pub rotation: Option<i32>,
...
#[strum(serialize_all = "snake_case")]
pub enum BuoyancySurfaceAttribute {
    Position,
    Orientation,
    Rotation,
    Shape,
...
impl GetAttributeValueRoot for BuoyancySurfaceAttribute {
    fn get_value_root(&self, d: &Definition) -> Vec<AttributeValue> {
        if let Some(buoyancy_surfaces) = d.buoyancy_surfaces.last() {
            buoyancy_surfaces
                .surface
                .iter()
                .filter_map(|item| self.get_value(item))
...
impl GetAttributeValue<BuoyancySurface> for BuoyancySurfaceAttribute {
    fn get_value(&self, d: &BuoyancySurface) -> Option<AttributeValue> {
        match self {
            Self::Position => Some((*d.position.last()?).into()),
            Self::Orientation => Some(d.orientation?.into()),
            Self::Rotation => Some(d.rotation?.into()),
...
impl From<BuoyancySurfaceAttribute> for AttributeSpecifier {
    fn from(value: BuoyancySurfaceAttribute) -> Self {
        Self::BuoyancySurface(value)
    }
}
