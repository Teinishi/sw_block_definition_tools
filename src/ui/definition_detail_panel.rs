        ...
        if let Ok(mut definition) = definition.lock() {
            if let Some(data) = definition.load_data() {
                ...
                if table.update(
                    ...
                    CollapsingPanel::new(&format!("Logic nodes ({})", table.len().unwrap_or(0)))
                        .ui(ui, |ui| {
                            table.ui(ui, state, &mut clicked_attribute);
                        });
                }
                ...
                return Some(AttributeDetailWindow::new(
                    clicked_attribute?,
                    state.hide_default,
                ));
            }
